...
void Go()//前进程序
{
  analogWrite(OUT1A, 230);  //将速度级别写入电机
  digitalWrite(OUT1B, LOW);
  analogWrite(OUT2A, 230); 
...
void Right()//右转程序
{
  ...
  analogWrite(OUT1B, 150);
  analogWrite(OUT2A, 220);
  digitalWrite(OUT2B, LOW);
}

void Left()//左转程序
{
  analogWrite(OUT1A,220); 
  digitalWrite(OUT1B, LOW);
  digitalWrite(OUT2A, LOW);
  analogWrite(OUT2B, 150);
}
void RightBack()//右后退程序
{
  digitalWrite(OUT1A, LOW);
  analogWrite(OUT1B, 200);
  digitalWrite(OUT2A, LOW);
  analogWrite(OUT2B, 150);
}
void LeftBack()//左后退程序
